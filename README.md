...
.
├── DATA/                       # 数据文件夹
│   ├── dataset.json            # 处理后的数据集
│   ├── News_Category_Dataset_v3.json # 原始数据集
│   └── glove.6B.100d.txt       # GloVe 词向量文件
├── Results/                    # 训练结果与模型权重保存目录
│   ├── first/                  # 基础模型权重
│   ├── second/                 # Advanced 模型权重
│   ├── glove/                  # GloVe 模型权重
│   └── bert/                   # BERT 模型权重
├── show/                       # 应用与展示脚本
│   ├── show_app.py             # 可视化分类应用
...
├── train_model/                # 训练笔记本目录
│   ├── NewsClassifier.ipynb    # 基础 Transformer 模型
│   ├── NewsClassifier_Advanced.ipynb # 高级模型 (RoPE, RMSNorm)
│   ├── NewsClassifier_BERT.ipynb     # BERT 微调模型
│   └── NewsClassifier_GloVe.ipynb    # GloVe + LSTM/CNN 模型
